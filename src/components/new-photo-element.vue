    ...
    <figure>
      <form>
        <label for="upload-input">
          <img role="presentation" ref="photoPreview" :src="
              srcPreview.length == 0 ? '/src/assets/svg/image.svg' : srcPreview
            " />
          <span>{{
            photoStatus == 0
            ? "upload file"
            : photoStatus == 1 || photoStatus == 4
            ? "change file"
            : ""
            }}</span>
          <input ref="fileInput" id="upload-input" type="file" v-on:change="setPreview" />
        </label>
        <button ref="fileSubmit" type="submit" @click="startUpload">
          <img src="/src/assets/svg/upload.svg" />
          {{ photoStatus == 1 ? "upload" : "try upload again" }}
        </button>
      </form>
      <figcaption>
        <ul class="photo-tag-list" ref="photoTagList">
          <photo-tag v-for="tag in tags" :key="tag" :tag="tag" class="edit"
            @click="removeTag(tag)" :buttonlabel="'remove '" :interactive="true" />
          <li class="add-tag edit" @click="$nextTick(() => $refs.tagInput.focus())">
            <form>
              <input ref="tagInput" type="text" size="1" placeholder="tag name" />
              <button type="submit" @click="addTag">
                ...
                </svg></button><!-- ; $nextTick(() => ($refs.tagInput.focus())) -->
            </form>
          </li>
        </ul>
      </figcaption>
...
export default {
  ...
  methods: {
    setPreview() {
      const previewSrc = window.URL.createObjectURL(
      ...
      this.$store.commit("photos/setStatus", {
        photoId: this.id,
    ...
    startUpload(event) {
      ...
      this.$store.dispatch("photos/uploadPhoto", {
        photoId: this.id,
        src: this.srcPreview,
      });
      this.$store.commit("photos/setStatus", {
        photoId: this.id,
        status: pStatus.UPLOADING,
      });
      this.$store.commit("photos/addNew");
    },
    removeTag(tagName) {
      // remove tag
      this.$store.commit("photos/removeTag", {
        photoId: this.id,
        tagName: tagName,
      });
      this.$store.commit("tags/removePhoto", {
        photoId: this.id,
        tagName: tagName,
      });
    },
    addTag(event) {
      ...
      if (tagName.length > 0) {
        this.$store.commit("photos/addTag", {
          photoId: this.id,
          tagName: tagName,
        });
        this.$store.commit(
          "tags/add", { tagName: tagName, photoId: this.id }, {
            root: true,
          }
        );
