<template>
  <li class="photo-element" :class="{
    ...
    <button class="edit-button" :class="{ edit: editing }" @click="toggleEditing">
      {{ editing ? "done" : "edit" }}
    </button>
    <figure>
      <img class="photo-img" :src="src" />
      <img role="presentation" v-if="photoStatus == 2 && srcPreview.length > 0"
        class="preview-img" :src="srcPreview" />
      <div v-if="photoStatus == 2" class="upload-note">
        <img role="presentation" src="/src/assets/svg/chevrons-up.svg" alt="" />
        <span>uploading...</span>
      </div>
      <button class="delete" :class="{ hidden: !editing }" label="delete photo"
        @click="deletePhoto">
      ...
      <figcaption>
        <ul class="photo-tag-list" ref="photoTagList">
          <photo-tag v-for="tag in tags" :key="tag" :tag="tag" :class="{ edit: editing }"
            :buttonlabel="editing ? 'filter by ' : 'remove '" :interactive="editing"
            @click="actionTag(tag)" />
          <li class="add-tag" :class="{ edit: editing }" @click="$nextTick(() => $refs.tagInput.focus())">
            <form>
              ...
              <button type="submit" @click="addTag">
                <img src="/src/assets/svg/plus.svg" alt="" @click="$nextTick(() => $refs.tagInput.focus())" />
              </button>
            </form>
          </li>
        </ul>
